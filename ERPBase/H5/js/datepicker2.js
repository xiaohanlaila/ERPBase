...
(function (window, document, Math) {
    ...
    _end: function () {
      ...
      self.scroller.addEventListener('touchend', function (e) {
        ...
        if( (self.offsetTop > 0) || (Math.abs(self.offsetTop) > Math.abs(self.scrollHeight)) ){
          ...
          self.offsetTop += offsetHeight * speed * 8;// 惯性移动距离
  ...
  function DatePicker(params) {
    ...
    if ( this.options.defaultValue ) {
      ...
      var date = defaultValue.split(" ")[0]
      var dvArray = date.split(separator);
      ...
      if (this.options.type == '4') {
        var time = defaultValue.split(" ")[1]
        var timeArr = time.split(':');
        if( timeArr.length > 0 ){
          for( var num = 0; num < timeArr.length; num++ ){
            this.defaultArray[this.defaultArray.length] = timeArr[num];
          }
        }
      }
  ...
  DatePicker.prototype = {
    ...
    FillData: function () {
      ...
      var picker_list = '<div class="zx_select showPicker">' +
        ...
        '<div class="ub" id="wrapper-parent"><div class="linear-mask"><div class="ub-f1 picker-wrapper" id="dp-wrapper0"><ul></ul></div>';
      ...
      if( self.options.type > 3 ){
        ...
        picker_list += '<div class="ub-f1 picker-wrapper" id="dp-wrapper4"><ul></ul></div>';
      }

      // picker_list += '<div class="sel_top"></div><div class="sel_bottom"></div>';
      picker_list += '<div class="sel_middle"></div>';
      picker_list += '</div></div></div>';
      ...
      switch(self.options.type){
        ...
        case 4:
          ...
          self.getMonthList(1, self.defaultArray[1]);
          self.getDayList(2, self.defaultArray[2]);
          self.getHList(3, self.defaultArray[3]);
          self.getMList(4, self.defaultArray[4]);
          break;
        default:
          self.getYearList(0, self.defaultArray[0]);
          break;
    ...
    //获取时列表
    getHList: function (index, defaultValue) {
      var self = this;
      var list = '<li></li><li></li>', defaultNum = '';
    ...
    //获取分列表
    getMList: function (index, defaultValue) {
      var self = this;
      var list = '<li></li><li></li>', defaultNum = '';
    ...
    eventClick: function () {
      ...
      document.querySelector('.picker-ok').addEventListener("touchstart", function (e) {
        ...
        if( self.options.callBack ){
          self.options.callBack(inputValue, self.textArray);
        }
    ...
    setDefaultOptions: function (params) {
      ...
      if( type && (type < 0 || type > 4) ){
        ...
        params.type = (type || type == 0) ? type : 3;
      }
      ...
      if( !params.defaultValue || params.defaultValue == "" ){
        if( params.type == 0 ){
          ...
          params.defaultValue = nowYear + params.separator + nowMonth + params.separator + nowDay + " " + nowH + ":" + nowM;
        }
      }
